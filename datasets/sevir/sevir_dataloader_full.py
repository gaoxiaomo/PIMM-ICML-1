...
PREPROCESS_OFFSET_SEVIR = {'vis': 0,  # Not utilized in original paper
                           ...
                           'vil': - 33.44,
                           'lght': - 0.02990}
PREPROCESS_SCALE_01 = {'vis': 1 / 2099.7733,
                       'ir069': 1 / 1174.68,
                       'ir107': 1 / 2562.43,
                       'vil': 1 / 255,
                       'lght': 1 / 0.60517}
PREPROCESS_OFFSET_01 = {'vis': -1459.9081,
                        'ir069': 3683.58,
                        'ir107': 1552.80,
                        'vil': 0,
                        'lght': -0.02990}
...
class SEVIRDataLoader:
    ...
    def __init__(self,
        ...
        assert sample_mode in ['random',
                               'sequent'], f'Invalid sample_mode = {sample_mode}, must be \'random\' or \'sequent\'.'
    ...
    def _compute_samples(self):
        ...
        filtcat = filtcat.groupby('id').filter(lambda x: set(x['img_type']).__contains__('vil'))  # 37935, 11972
        filtcat = filtcat.groupby('id').filter(lambda x: x.shape[0] == len(set(x['img_type'])))  # 37614, 11906
        self._samples = filtcat.groupby('id').apply(lambda df: self._df_to_series(df))
        if self.shuffle:
            self.shuffle_samples()
    ...
    def _df_to_series(self, df0):
        d = {}
        imgt = set(df0['img_type'])
        df = df0.set_index('img_type')
        for i in imgt:
            s = df.loc[i]
    ...
    def _open_files(self, verbose=True):
        ...
        Opens HDF files
        """
        imgt = ['vis', 'ir069', 'ir107', 'vil', 'lght']
        hdf_filenames = []
        for t in imgt:
            hdf_filenames += list(np.unique(self._samples[f'{t}_filename'].dropna().values))
        self._hdf_files = {}
    ...
    def _read_data(self, row, data):
        ...
        """
        valid_types = np.unique([x.split('_')[0] for x in list(row.dropna().keys())])
        for t in SEVIR_DATA_TYPES:
            if t in valid_types:
                fname = str(row[f'{t}_filename'])
                t_slice = slice(0, None)
                # Need to bin lght counts into grid
                if t == 'lght':
                    idx = str(row[f'{t}_index'])
                    lght_data = self._hdf_files[fname][idx][:]
                    data_i = self._lght_to_grid(lght_data, t_slice)
                else:
                    idx = int(row[f'{t}_index'])
                    data_i = self._hdf_files[fname][t][idx:idx + 1, :, :, t_slice]
            elif t == 'vis':
                data_i = np.zeros((1, 768, 768, 49), dtype=float)
            elif t == 'ir069' or t == 'ir107':
                data_i = np.zeros((1, 192, 192, 49), dtype=float)
            elif t == 'lght':
                data_i = np.zeros((1, 48, 48, 49), dtype=float)
            else:
    ...
    def _lght_to_grid(self, data, t_slice=slice(0, None)):
        ...
        out_size = (*self.data_shape['lght'], len(self.lght_frame_times)) if t_slice.stop is None else (
            *self.data_shape['lght'], 1)
        if data.shape[0] == 0:
    ...
    def _load_event_batch(self, event_idx, event_batch_size):
        ...
        event_valid_flags = {}
        for e_idx in range(event_idx, event_idx_slice_end):
            event_valid_flags[str(e_idx)] = {}
            for t in SEVIR_DATA_TYPES:
                is_invalid = pd.isnull(self._samples.iloc[event_idx][t + '_index']) or \
                             pd.isnull(self._samples.iloc[event_idx][t + '_filename'])
                event_valid_flags[str(e_idx)][t] = not is_invalid
        ...
        if pad_size > 0:
            ...
            event_batch = [data[t].astype(self.output_type) for t in data_types]
        return event_batch, event_valid_flags
    ...
    def _random_sample(self):
        ...
        while num_sampled < self.batch_size:
            ...
            for imgt_idx, imgt in enumerate(self.data_types):
                sampled_seq = event[imgt_idx][[0, ], :, :,
                              seq_slice_list[num_sampled]]  # keep the dim of batch_size for concatenation
                if imgt in ret_dict:
    ...
    def _idx_sample(self, index):
        ...
        data_types = SEVIR_DATA_TYPES
        event_batch, event_valid_flags = \
            self._load_event_batch(event_idx=start_event_idx, event_batch_size=event_batch_size)

        ret_dict = {}
        valid_flags = []
        for sampled_idx in sampled_idx_list:
            e_idx = str(sampled_idx['event_idx'])
            valid_flags.append(event_valid_flags[e_idx])
            batch_slice = [sampled_idx['event_idx'] - start_event_idx, ]  # use [] to keepdim
            seq_slice = slice(sampled_idx['seq_idx'] * self.stride,
                              sampled_idx['seq_idx'] * self.stride + self.seq_len)
            for imgt_idx, imgt in enumerate(data_types):
                # if imgt == 'lght':
                #     sampled_seq = event_batch[imgt_idx]
                # else:
                #     sampled_seq = event_batch[imgt_idx][batch_slice, :, :, seq_slice]
                sampled_seq = event_batch[imgt_idx][batch_slice, :, :, seq_slice]
                if imgt in ret_dict:
        ...
        if self.downsample_dict is not None:
            ret_dict = self.downsample_data_dict(data_dict=ret_dict,
                                                 ...
                                                 factors_dict=self.downsample_dict,
                                                 layout=self.layout)
        ret_dict['valid_flags'] = valid_flags
        batches = len(valid_flags)
        for t in SEVIR_DATA_TYPES:
            if ret_dict[t].shape[0] != batches:
                print('VALID_FLAGS:')
                print(valid_flags)
                for t1 in SEVIR_DATA_TYPES:
                    print(t1, str(ret_dict[t].shape))
                raise ValueError('dataloader exception')
        return ret_dict
