...
class WeatherBenchDataset(Dataset):
    ...
    def __init__(self, data_root, data_name, training_time,
        ...
        if 'mv' in data_name:  # multi-variant version
            ...
            for name in self.data_name:
                ...
                self.std.append(std)
            min_length = min(d.shape[0] for d in self.data)
    ...
    def _load_data_xarray(self, data_name, single_variant=True):
        ...
        if data_name != 'uv10':
            ...
            if not single_variant and isinstance(self.level, list) and data_name != 'tp':
    ...
    def center_crop(data, crop_size):
        """Center crop the data to the given crop_size.

        Args:
            data (torch.Tensor): The data tensor with shape (C, H, W).
