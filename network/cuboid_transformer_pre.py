...
class PhyEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        if phy_shapes is None:
            phy_shapes = {
                ...
                'rf': ((3, 32, 64), (16, 32, 64), (32, 32, 64)),
                # 't': ((3, 32, 64), (16, 32, 64), (32, 32, 64)),
                'uf': ((3, 32, 64), (16, 32, 64), (32, 32, 64)),
                'vf': ((3, 32, 64), (16, 32, 64), (32, 32, 64))
            }
        now_shapes = {
            'r': ((3, 32, 64), (16, 32, 64), (32, 32, 64)),
            # 't': ((3, 32, 64), (16, 32, 64), (32, 32, 64)),
        ...
        self.now_types = list(now_shapes.keys())
        self.future_types = list(future_shapes.keys())
    ...
    def forward(self, x):
        ...
        for k in self.phy_shapes.keys():
            ...
            for i in range(1, len(self.phy_shapes[k])):
                bt, c, h, w = p.shape
...
class CuboidTransformerModel(nn.Module):
    ...
    def forward(self, x, y, verbose=False):
        ...
        phy_features = self.phy_encoder(x)
        ...
        for k in self.phy_encoder.now_types:
            features += phycell_out[k]
        features_all = self.correlation(features)
        features = features_all[:, :, :256, :, :]
        future_mid = torch.zeros_like(phy_features['rf'])
        for k in self.phy_encoder.future_types:
            future_mid += phycell_out[k]
         # loss_future = F.mse_loss(others, future_mid)
        loss_future = 0
        ...
        feature11 = out.permute(0, 1, 4, 2, 3).mean(dim=2).unsqueeze(2)
        # self.printlogs('./params/')
        return out, (mse_loss, phy_loss, phycell_loss, loss_future), none
