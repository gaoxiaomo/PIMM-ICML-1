...
from .correlation import Correlation_V2, Correlation_V1, Correlation_V3
from .cuboid_transformer_patterns import CuboidSelfAttentionPatterns, CuboidCrossAttentionPatterns
from .phydnet_models import PhyCell, K2M
from .utils import (
    get_activation, get_norm_layer,
    _generalize_padding, _generalize_unpadding,
    apply_initialization, round_to, AFNONet)
from ..datasets.sevir.sevir_dataloader import PREPROCESS_OFFSET_01, PREPROCESS_SCALE_01


def cal_vil_binary_loss(pred, target, tau=0.01, seg_offset=0.5, ratio_index=2):
    scale = PREPROCESS_SCALE_01['vil']  # 1 / 255
    offset = PREPROCESS_OFFSET_01['vil']  # 0
    # segments = (0, 16, 74, 133, 160, 181, 219, 256)
    eps = 1e-6
    ...
    for T in thresholds:
        ...
        tmp_loss = 1 - torch.mean(csi, dim=0)
        if (target >= T).float().sum() <= 0:
            tmp_loss = 0
        loss += tmp_loss
    loss /= len(thresholds)
    ratio = (target >= thresholds[0]).float().sum() / target.numel()
    loss = loss * (1 + 0.5 * torch.sin(ratio * math.pi / 2))
    return loss
...
class DynamicConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_sizes):
        ...
        for size in kernel_sizes:
            padding = size // 2  # 使用 // 运算符确保得到整数值  ÷向下取整
            self.convs.append(
                nn.Conv2d(in_channels, out_channels // len(kernel_sizes), kernel_size=size, stride=1, padding=padding)
            )

    def forward(self, x):
        # 计算注意力权重
        gating_weights = self.avg_pool(x)
        gating_weights = gating_weights.view(gating_weights.size(0), -1)
        gating_weights = self.gate(gating_weights)
        # gating_weights = F.softmax(gating_weights, dim=1)
        # soft weights
        gating_weights = F.softmax(gating_weights / self.tau, dim=1)
        Batch, _ = gating_weights.shape
        ...
        for idx, conv in enumerate(self.convs):
            conv_out = conv(x)
            # 使用注意力权重动态组合卷积核输出
            conv_out = conv_out * gating_weights[:, idx:idx + 1].view(Batch, 1, 1, 1)
...
class PhyEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        if phy_shapes is None:
            phy_shapes = {
                'r': ((3, 32, 64), (16, 32, 64), (32, 32, 64)),
              #  't': ((3, 32, 64), (16, 32, 64), (32, 32, 64)),
                'u': ((3, 32, 64), (16, 32, 64), (32, 32, 64)),
                'v': ((3, 32, 64), (16, 32, 64), (32, 32, 64))
            }
        self.t_in = t_in
        self.phy_shapes = phy_shapes
        self.phy_types = list(phy_shapes.keys())
        self.init_encoders = nn.ModuleDict()
        for k in phy_shapes.keys():
            convs = nn.ModuleList()
            for i in range(1, len(phy_shapes[k])):
                c_in = phy_shapes[k][i - 1][0]
                c_out = phy_shapes[k][i][0]
                convs.append(nn.Sequential(
                    nn.Conv2d(c_in, c_out, kernel_size=(3, 3), stride=1, padding=1),
                    nn.LayerNorm([c_out, phy_shapes[k][i][1], phy_shapes[k][i][2]]),
                    nn.LeakyReLU(0.1),
                    nn.Dropout(0.3)
                ))
            self.init_encoders.add_module(k, convs)
        self.corr = nn.ModuleDict()
        for k in phy_shapes.keys():
            in_channels = t_in * phy_shapes[k][-1][0]
            out_channels = t_in * 32
            self.corr.add_module(k, nn.Sequential(
                nn.Conv2d(in_channels, out_channels, kernel_size=(1, 1), stride=1, padding=0),
                nn.LayerNorm([out_channels, phy_shapes[k][-1][1], phy_shapes[k][-1][2]]),
                nn.LeakyReLU(0.1),
                nn.Dropout(0.3)
            ))

    def forward(self, x, mode='0'):
        # input: x[k] = (B, T, H0, W0, C0), output: x[k] = (B, T, C1, H1, W1)
        B, T, _, _, C0 = x[self.phy_types[0]].shape
        phy_features = {}
        for k in self.phy_shapes.keys():
            p = x[k]  # (B, T, H, W, C)
            p = einops.rearrange(p, 'b t h w c -> (b t) c h w')
            if mode == '0':
                start_idx = 1
            elif mode == '1':
                start_idx = len(self.phy_shapes[k]) - 1
            else:
                return None
            for i in range(start_idx, len(self.phy_shapes[k])):
                bt, c, h, w = p.shape
                c_next, h_next, w_next = self.phy_shapes[k][i]
                if h > h_next:
                    r = h // h_next
                    p = einops.rearrange(p, 'bt c (h r) w -> bt (c r) h w', r=r)
                elif h < h_next:
                    r = h_next // h
                    p = einops.rearrange(p, 'bt (c r) h w -> bt c (h r) w', r=r)
                if w > w_next:
                    r = w // w_next
                    p = einops.rearrange(p, 'bt c h (w r) -> bt (c r) h w', r=r)
                elif w < w_next:
                    r = w_next // w
                    p = einops.rearrange(p, 'bt (c r) h w -> bt c h (w r)', r=r)
                p = self.init_encoders[k][i - 1](p)

            p = einops.rearrange(p, '(b t) c h w -> b (t c) h w', b=B)
            p = self.corr[k](p)
            p = einops.rearrange(p, 'b (t c) h w -> b t c h w', t=self.t_in)
            phy_features[k] = p
        return phy_features

class Constraits(nn.Module):
    def __init__(self):
        super(Constraits, self).__init__()
        self.phycell_constraits = torch.zeros((49, 7, 7))
        ind = 0
        for i in range(0, 7):
            for j in range(0, 7):
                self.phycell_constraits[ind, i, j] = 1
                ind += 1

    def forward(self, m):
        # 生成constrained_parameters矩阵
        constrained_parameters = self.phycell_constraits.clone()  # 使用clone确保不改变原参数

        # 只对前28个矩阵的左上三角进行约束
        upper_left_m = torch.triu(m[:28, :7, :7])  # 获取m的上三角部分
        upper_left_constrained = torch.triu(constrained_parameters[:28, :7, :7])  # 获取constrained_parameters的上三角部分

        return F.mse_loss(upper_left_m, upper_left_constrained.to(m.device))
...
class CuboidTransformerModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.down_dim = 16
        self.phycell_constraits = Constraits()
        ...
        self.get_initial_encoder_final_decoder(
    ...
    def get_initial_encoder_final_decoder(
        ...
        if self.initial_downsample_type == "conv":
            ...
            for i in range(len(initial_downsample_stack_conv_dim_list)):
                initial_downsample_stack_conv_dim_list[i] //= 2

            self.initial_encoder1 = InitialStackPatchMergingEncoder(
            ...
            self.init_conv = nn.Sequential(
                nn.Conv2d(32 * T_in, 32 * T_in, kernel_size=(1, 1), stride=1),
                nn.LayerNorm([32 * T_in, 32, 64]),
                nn.LeakyReLU(0.1),
                # nn.Dropout(0.1)
            )
            #x
            x1 = einops.repeat(torch.linspace(1, 64, 64), 'w -> h w', h=32)
            x2 = einops.repeat(torch.linspace(2, 128, 64), 'w -> h w', h=32)
            x3 = einops.repeat(torch.linspace(4, 256, 64), 'w -> h w', h=32)
            #y
            y1 = einops.repeat(torch.linspace(1, 32, 32), 'h -> h w', w=64)
            y2 = einops.repeat(torch.linspace(2, 64, 32), 'h -> h w', w=64)
            y3 = einops.repeat(torch.linspace(4, 128, 32), 'h -> h w', w=64)
            self.x_params = {
                'r': (x1 - torch.mean(x1)) / torch.std(x1),
                'u': (x2 - torch.mean(x2)) / torch.std(x2),
                'v': (x3 - torch.mean(x3)) / torch.std(x3),
            }
            self.y_params = {
                'r': (y1 - torch.mean(y1)) / torch.std(y1),
                'u': (y2 - torch.mean(y2)) / torch.std(y2),
                'v': (y3 - torch.mean(y3)) / torch.std(y3),
            }

            self.m_params = nn.ParameterDict({
                'r': nn.Parameter(torch.randn([1])),
                'u': nn.Parameter(torch.randn([1])),
                'v': nn.Parameter(torch.randn([1])),
            })
            self.t_param_conv = nn.Sequential(
                nn.Linear(32, 32 * 4),
                nn.Dropout(0.3)
            )
            self.u_conv = nn.Sequential(
                nn.Conv2d(9+32, 32, kernel_size=(3, 3), stride=1, padding=1),
                nn.LayerNorm([32, 32, 64]),
                nn.LeakyReLU(0.1),
                nn.Dropout(0.3)
            )

            self.phy_encoder = PhyEncoder()

            # self.halluci_loss = nn.KLDivLoss()
            self.halluci_loss = nn.MSELoss()

            self.correlation = Correlation_V3(in_channels=32 * T_in, hid_channels=256,
                                              last_channels=(256 + 32) * T_in, N2=3)
            self.phy_h = 32
            self.phy_w = 64
            self.phy_dim = 32
            self.phycell = PhyCell(input_shape=(self.phy_h, self.phy_w), input_dim=self.phy_dim,
                                   F_hidden_dims=[49], n_layers=1, kernel_size=(7, 7))
            self.phycell_k2m = K2M((7, 7))
            # use `self.target_shape` to get correct T_out
            dec_target_shape_list = [(4, 32, 64, 32), (4, 32, 64, 16)]
            dec_in_dim = 256
            self.final_decoder = FinalStackUpsamplingDecoder(
                ...
                linear_init_mode=self.down_up_linear_init_mode,
                norm_init_mode=self.norm_init_mode)
            dec_out_dim = dec_target_shape_list[-1][-1]
            self.final_conv = nn.Sequential(
                nn.Conv2d(dec_out_dim, dec_out_dim, kernel_size=(1, 1), stride=1, padding=0),
                nn.LayerNorm([dec_out_dim, 32, 64], elementwise_affine=False),
                nn.LeakyReLU(0.1),
                # nn.Dropout(0.4)
            )
    ...
    def reset_parameters(self):
        ...
        if hasattr(self.initial_encoder1, "reset_parameters"):
            self.initial_encoder1.reset_parameters()
        else:
            apply_initialization(self.initial_encoder1,
                                 conv_mode=self.conv_init_mode,
                                 linear_mode=self.down_up_linear_init_mode,
                                 norm_mode=self.norm_init_mode)
        # if hasattr(self.initial_encoder2, "reset_parameters"):
        #     self.initial_encoder2.reset_parameters()
        # else:
        #     apply_initialization(self.initial_encoder2,
        #                          conv_mode=self.conv_init_mode,
        #                          linear_mode=self.down_up_linear_init_mode,
        #                          norm_mode=self.norm_init_mode)
    ...
    def per(self, x, y):
        tensor1_flat = x.flatten()
        tensor2_flat = y.flatten()
        ...
        # 计算协方差
        cov = torch.sum((tensor1_flat - mean1) * (tensor2_flat - mean2))

        # 计算标准差
        std1 = torch.sqrt(torch.sum((tensor1_flat - mean1) ** 2))
        std2 = torch.sqrt(torch.sum((tensor2_flat - mean2) ** 2))
    ...
    def forward(self, x, y, verbose=False):
        ...
        """
        B, T, _, _, _ = x['t'].shape
        init_feature = self.initial_encoder1((x['t'])).permute(0, 1, 4, 2, 3)  # (B, 13, 64, 32, 32)

        features = einops.rearrange(init_feature, 'b t c h w -> b (t c) h w')
        features = self.init_conv(features)
        features = einops.rearrange(features, 'b (t c) h w -> b t c h w', t=T)

        x1, delta_t, phy_features = {}, [], {}
        for t in range(T - 1):
            dt = init_feature[:, t+1, :, :, :] - init_feature[:, t, :, :, :]
            dt = self.t_param_conv(einops.rearrange(dt, 'b c h w -> b h w c'))
            dt = einops.rearrange(dt, 'b h w c -> b c h w')
            delta_t.append(torch.chunk(dt, 4, dim=-3))
        for i, k in enumerate(['r', 'u', 'v']):
            _m = einops.repeat(self.m_params[k], 'h -> b c (repeat h) w', b=B, c=3, repeat=32, w=64).to(x['t'].device)
            _x = einops.repeat(self.x_params[k], 'h w -> b c h w', b=B, c=3).to(x['t'].device)
            _y = einops.repeat(self.y_params[k], 'h w -> b c h w', b=B, c=3).to(x['t'].device)
            _t0 = init_feature[:, 0, :, :, :]
            _u0 = torch.cat([_x, _y, _m, _t0], dim=-3)  # (b, c=3+64, w=32, h=32)

            _u0 = self.u_conv(_u0)  # (b, c=32, w=32, h=32)
            _u = [_u0]
            for t in range(T - 1):
                _u0 = _u0 + delta_t[t][i]
                _u.append(_u0)
            p = einops.rearrange(torch.stack(_u, dim=1), 'b t c h w -> b (t c) h w')
            p = self.phy_encoder.corr[k](p)
            p = einops.rearrange(p, 'b (t c) h w -> b t c h w', t=T)
            phy_features[k] = p

        all_valid = True
        halluci_loss = 0.0
        rt = self.per(features, phy_features['r'])
        vt = self.per(features, phy_features['v'])
        ut = self.per(features, phy_features['u'])
        rv = self.per(phy_features['r'], phy_features['v'])
        ru = self.per(phy_features['r'], phy_features['u'])
        uv = self.per(phy_features['u'], phy_features['v'])
        """
        if all_valid:
            pre_phy = self.phy_encoder(x)
            phy_feature_0 = 0.0
            for k in pre_phy.keys():
                phy_feature_0 += pre_phy[k]
            halluci_loss = self.halluci_loss(phy_feature, phy_feature_0)
        """
        phycell_out = {}
        for k in self.phy_encoder.phy_types:
            phycell_mid = []
            for t in range(T):
                first_timestep = (t == 0)
                phycell_mid.append(self.phycell(phy_features[k][:, t, :, :, :], first_timestep)[0][-1])
            phycell_out[k] = torch.stack(phycell_mid, dim=1)
        loss_rv = F.mse_loss(phycell_out['r'], phycell_out['v'])
        loss_uv = F.mse_loss(phycell_out['u'], phycell_out['v'])
        loss_ru = F.mse_loss(phycell_out['r'], phycell_out['u'])
        phy_loss = loss_rv + loss_uv + loss_ru
        phycell_loss = 0
        phycell_loss = 0
        for b in range(0, self.phycell.cell_list[0].input_dim):
            filters = self.phycell.cell_list[0].F.conv1.weight[:, b, :, :]
            m = self.phycell_k2m(filters.double())
            m = m.float()
            mid_loss = self.phycell_constraits(m)
            phycell_loss += mid_loss
        for k in self.phy_encoder.phy_types:
            features += phycell_out[k]
        features_all = self.correlation(features)
        features = features_all[:, :, :256, :, :]
        others = features_all[:, :, 256:, :, :]
        features = einops.rearrange(features, 'b t c h w -> b t h w c')

        dec_out = self.final_decoder(features)

        dec_out = einops.rearrange(dec_out, 'b t h w c -> (b t) c h w')
        dec_out = self.final_conv(dec_out)
        dec_out = einops.rearrange(dec_out, '(b t) c h w -> b t h w c', b=B)

        T_out = self.target_shape[0]
        out = self.dec_final_proj(dec_out)[:, :T_out, :, :, :]

        binary_loss = cal_vil_binary_loss(out, y)
        mse_loss = F.mse_loss(out, y)

        # self.printlogs('./params/')

        return out, (mse_loss, halluci_loss,rt,vt,ut,rv,ru,uv,phycell_loss,phy_loss)

    def printlogs(self, logdir):
        ...
        if self.I == 5:
            with open(logdir + 'params.txt', 'w') as f:
                params = ['phycell', 'phy_gt_conv', 'phy_final_conv', 'shared_convs', 'shared_corr_convs']
                for p in params:
                    f.write(p + ': \n')
                    model = getattr(self, p)
                    for name, param in model.named_parameters():
                        mean_v = round(param.mean().to('cpu').item(), 6)
                        max_v = round(param.max().to('cpu').item(), 6)
                        min_v = round(param.min().to('cpu').item(), 6)
                        f.write(name + ': ' + str(mean_v) + ', ' + str(max_v) + ', ' + str(min_v) + '\n')
                    f.write('\n\n')
        if self.I > 0:
            items = set(dir(self))
            for n in items:
                ...
                item = getattr(self, n)
                n = str(n)[4:]
                mean_v = round(item.mean().to('cpu').item(), 6)
                self.WRITTER.add_scalar(n, mean_v, global_step=self.I)
        if self.I > 205:
            self.WRITTER.close()
            print('\n---END---\n')
            sys.exit()
